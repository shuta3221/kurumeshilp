(() => {
  ...
  function recommendation(a) {
    ...
    if (a[1] === 2 || a[3] === 3) return ['法人利用の総合相談', '現在のお弁当の注文方法や請求処理を伺い、注文代行・請求書の取りまとめ・利用実績の集計から、必要な対応をご案内します。', '利用方法を相談する', 'https://kurumeshi.tayori.com/f/hojin/'];
    const bulk = a[1] === 1;
    const management = a[3] === 1 || a[3] === 2;
    if (bulk && management) return ['大口発注＋管理のご相談', 'くるめしがお弁当選びと注文を代行します。部署・従業員ごとの利用実績の集計や予算との比較も、ご希望に応じて個別に対応します。', '大口発注・管理についてまとめて相談する →', 'https://kurumeshi.tayori.com/f/hojin/'];
    if (management) {
      const payment = ['請求書をまとめる範囲も確認します。', '立替払いを減らすため、請求書による精算方法もご案内します。', '', '支払い方法についてもご相談いただけます。'][a[2]];
      const reporting = a[3] === 2 ? 'ご希望に応じて利用実績を集計し、予算と比較するレポートを個別に作成します。' : 'ご希望に応じて部署・従業員ごとの利用額を集計し、レポートを個別に作成します。';
      return ['管理向けサービスのご相談', reporting + '集計項目や提供頻度は、ご相談時に確認します。' + payment, '管理について相談する', 'https://kurumeshi.tayori.com/f/hojin/'];
    }
    if (bulk) return ['大口発注向けサービスのご相談', 'くるめしコンシェルジュが利用シーンに合うお弁当を提案し、大量・継続的な注文を代行します。', '大口発注について相談する', 'https://kurumeshi.tayori.com/f/hojin/'];
    ...
    return ['法人利用の総合相談', '現在のお弁当の注文方法や請求処理を伺い、注文代行・請求書の取りまとめ・利用実績の集計から、必要な対応をご案内します。', '利用方法を相談する', 'https://kurumeshi.tayori.com/f/hojin/'];
  }
  function render(focus = true) {
    ...
    if (step < 4) {
      ...
      if (result[3] === 'https://kurumeshi.tayori.com/f/hojin/') {
