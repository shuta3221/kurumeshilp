(() => {
  ...
  function recommendation(a) {
    ...
    if (a[1] === 2 || a[3] === 3) return ['法人利用の総合相談', 'まだ決まっていない手配や利用管理のご希望を含め、今の利用方法に合うサポートを一緒に整理します。', '利用方法を相談する', 'https://kurumeshi.tayori.com/f/hojin/'];
    const bulk = a[1] === 1;
    const management = a[3] === 1 || a[3] === 2;
    if (bulk && management) return ['大口発注＋管理のご相談', '手配の支援と、社内の利用管理の両方をご希望のため、まとめてご相談いただく方法がおすすめです。', '大口発注・管理についてまとめて相談する →', 'https://kurumeshi.tayori.com/f/hojin/'];
    if (management) {
      const payment = ['請求の集約も含めて、', '立替・領収書処理の負担軽減も含めて、', '', '支払い方法のご相談も含めて、'][a[2]];
      return ['管理向けサービスのご相談', '利用実績の把握や予算管理をご希望のため、' + payment + '集計単位や運用方法から相談できる窓口をご案内します。', '管理について相談する', 'https://kurumeshi.tayori.com/f/hojin/'];
    }
    if (bulk) return ['大口発注向けサービスのご相談', '大量・継続的な手配の支援をご希望のため、お弁当選びや発注を相談できる窓口がおすすめです。', '大口発注について相談する', 'https://kurumeshi.tayori.com/f/hojin/'];
    ...
    return ['法人利用の総合相談', '今の利用方法やお困りごとを伺い、必要なサポートを一緒に整理します。', '利用方法を相談する', 'https://kurumeshi.tayori.com/f/hojin/'];
  }
