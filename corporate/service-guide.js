(() => {
  ...
  function recommendation(a) {
    ...
    if (bulk && management) return ['大口発注＋管理のご相談', '手配の支援と、社内の利用管理の両方をご希望のため、まとめてご相談いただく方法がおすすめです。', '法人利用について相談する', '#contact'];
    if (management) return ['管理向けサービスのご相談', '利用実績の把握や予算管理をご希望のため、集計単位や運用方法から相談できる窓口をご案内します。', '管理について相談する', '#contact'];
  ...
  function render(focus = true) {
    ...
    if (step < 4) {
      ...
      const title = el('h3', result[0]); title.tabIndex = -1; host.append(title, el('p', result[1]));
      ...
      const link = el('a', result[2], 'button'); link.href = result[3];
      link.onclick = () => {
